...
public class DogAdonisHook {
   ...
   public static ItemStack freeze(ItemStack item) {
      if (!available()) {
         ...
         try {
            ...
            meta.getPersistentDataContainer().set(new NamespacedKey(Bukkit.getPluginManager().getPlugin("StainCrates"), "frozen_remaining"), PersistentDataType.LONG, remaining);
   ...
   public static ItemStack unfreeze(ItemStack item) {
      if (!available()) {
         ...
         NamespacedKey key = new NamespacedKey(Bukkit.getPluginManager().getPlugin("StainCrates"), "frozen_remaining");
         ItemMeta meta = item.getItemMeta();
